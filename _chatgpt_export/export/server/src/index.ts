...
// REST endpoints for config management
...
io.on('connection', (socket) => {
  ...
  // Create room
  socket.on('create_room', ({ playerName, partyName, configOverrides, socialIdeology, economicIdeology }) => {
    try {
      const roomId = roomManager.createRoom(configOverrides);
      const result = roomManager.joinRoom(roomId, socket.id, playerName, partyName, socialIdeology, economicIdeology);
  ...
  // Join room
  socket.on('join_room', ({ roomId, playerName, partyName, socialIdeology, economicIdeology }) => {
    try {
      const result = roomManager.joinRoom(roomId, socket.id, playerName, partyName, socialIdeology, economicIdeology);
  ...
  socket.on('send_chat', ({ content, recipientId }) => {
    ...
    try {
      ...
      if (recipientId) {
        ...
        // Global message - send to everyone in room
